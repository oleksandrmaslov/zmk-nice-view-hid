 ...
 */
 ...
 #include <zmk/split/transport/types.h>   // for struct zmk_split_transport_central_command
 #include <raw_hid/events.h>              // for raw_hid_received_event & raise_raw_hid_received_event()

 // 1) Command‐handler: fire a raw_hid_received_event when central sends RAW_HID packets
 static int raw_hid_command_handler(const struct zmk_split_transport_peripheral *transport,
 ...
 // 2) We don’t need to send any peripheral‐to‐central events here, so report_event can be NULL.
 static const struct zmk_split_transport_peripheral_api periph_transport_api = {
     .report_event = NULL,
 };

 // 3) Register our transport extension so that the core command‐handler will invoke it
 ZMK_SPLIT_TRANSPORT_PERIPHERAL_REGISTER(raw_hid_periph, &periph_transport_api);

 // 4) Finally, hook our command handler into ZMK’s dispatcher
 //    Behind the scenes, split/bluetooth/peripheral.c will call this for every cmd.
 int zmk_split_transport_peripheral_command_handler(
     const struct zmk_split_transport_peripheral *transport,
     struct zmk_split_transport_central_command cmd)
 {
     // First, let the core do its built‐in handling (key events, sensors, etc.)
     int ret = __zmk_split_transport_peripheral_command_handler(transport, cmd);

     // Then feed our RAW_HID packets through
     raw_hid_command_handler(transport, cmd);
     return ret;
 }
